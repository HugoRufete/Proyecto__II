...
public class Hacha : MonoBehaviour
{
    ...
    private void Attack()
    {
        animator.SetBool("HachaAttack", true);
        StartCoroutine(ResetHachaAttackAfterDelay());
    ...
    private IEnumerator ResetHachaAttackAfterDelay()
    {
        // Espera 0.5 segundos
        ...
        if (animator != null)
        {
            animator.SetBool("HachaAttack", false);
        }
    }
}
