...
public class GameManager : MonoBehaviour
{
    ...
    public int roundsPerType6 = 12; // Cada cuántas rondas aparece una oleada de tipo 6
    ...
    void SpawnEnemies(int waveType)
    {
        ...
        if (waveType == 1)
        {
            for (int i = 0; i < totalEnemiesInWave_1; i++)
            {
                if (RondaActual > 5)
                {
                    enemySpawner.SpawnGuadañaEnemy();  // A partir de la ronda 6, instanciar enemigos Firefly en lugar de BasicEnemy
                }
                else
                {
